...
class GameState {
  ...
  GameState({
    ...
    this.level = 1,
    Timings? timings,
    List<PrayerLog>? prayerLog,
    StreakState? heroStreak,
    Map<String, StreakState>? perPrayerStreaks,
    StreakState? tilawahStreak,
    List<Quest>? quests,
    this.questDate = '',
  ...
  factory GameState.fromMap(Map<String, dynamic> m) {
    final logList =
        (m['prayerLog'] as List?)
            ?.map((e) => PrayerLog.fromMap(e as Map<String, dynamic>))
            .toList() ??
        [];
    final questList =
        (m['quests'] as List?)
    ...
    return GameState(
      ...
      timings: m['timings'] != null
          ...
          : Timings(),
      prayerLog: logList,
      heroStreak: m['heroStreak'] != null
          ? StreakState.fromMap(m['heroStreak'] as Map<String, dynamic>)
          : null,
      perPrayerStreaks: pstr,
      tilawahStreak: m['tilawahStreak'] != null
          ? StreakState.fromMap(m['tilawahStreak'] as Map<String, dynamic>)
          : null,
      quests: questList,
...
class GameService {
  ...
  @visibleForTesting
  static int? debugChestPickIndex;
  ...
  static Future<bool> equipCosmetic(
    ...
    final next = CosmeticService.equip(
      _cache,
      slot: slot,
      id: id,
      isPro: isPro,
  ...
  static int xpNeededForLevel(int level) =>
      (40 + 8 * level + 0.5 * level * level).round();
  ...
  static String getRankTitle(int level) {
    String roman(int n) =>
        const {1: 'I', 2: 'II', 3: 'III', 4: 'IV', 5: 'V'}[n] ?? '';
    if (level >= 1 && level <= 9) {
  ...
  static bool isSunnahOnTime(String prayer, Timings t) {
    ...
    switch (prayer) {
      case 'dhuha':
        return isAfter(now, addMin(t.terbit, 15)) && isBefore(now, t.dzuhur);
      case 'tahajjud':
      ...
      case 'rawatib_dzuhur_ba_diyyah':
        return isAfter(now, t.dzuhur) && isBefore(now, t.ashar);
      case 'rawatib_ashar_qobliyah':
        return isAfter(now, t.ashar) && isBefore(now, t.maghrib);
  ...
  static StreakState _updWeeklyStreak(StreakState s, String today) {
    ...
    try {
      final diff = DateTime.parse(
        today,
      ).difference(DateTime.parse(s.lastDate)).inDays;
  ...
  static bool _isFriday(String dateStr) {
    try {
      return DateTime.parse(dateStr).weekday == DateTime.friday;
    } catch (_) {
      return false;
    }
  }
    ...
    return current.map((q) {
      ...
      switch (q.id) {
        ...
        case 'quest_five_rings':
          if (wajibList.every((p) => todayLogs.any((l) => l.prayer == p))) {
            prog = 1;
            done = true;
          }
          break;
  ...
  static (GameState, int, int)? logPrayer(
    ...
    final isHeroCompletor =
        ...
        wajibList.every(
          (p) =>
              p == prayer ||
              updatedLogs.any((l) => l.date == today && l.prayer == p),
        );
    ...
    final zikirNow = state.zikirCounter.date == today
  ...
  static Future<GameState> unlogPrayer(String prayer) async {
    ...
    final wasFullBefore =
        logItem.type == 'wajib' &&
        wajibList.every(
          (p) => _cache.prayerLog.any((l) => l.date == today && l.prayer == p),
        );
    ...
    if (wasFullBefore) {
      hero = hero.copyWith(
        current: (hero.current - 1).clamp(0, 999999),
        lastDate: '',
      );
    }
    if (prayer == 'tilawah') {
      tilawah = tilawah.copyWith(
        current: (tilawah.current - 1).clamp(0, 999999),
        lastDate: '',
      );
    }
  ...
  static Future<(GameState, int)> claimQuest(String questId) async {
    ...
    final quests = _cache.quests
        .map((x) => x.id == questId ? x.copyWith(claimed: true) : x)
        .toList();
  ...
  static List<Quest> generateQuestPool() {
    final pool = [
      Quest(
        id: 'quest_subuh_tepat',
        desc: 'Sholat Subuh tepat waktu (≤30 menit setelah adzan)',
        xpReward: 50,
        target: 1,
        progress: 0,
        completed: false,
      ...
      Quest(
        id: 'quest_dhuha_before_dzuhur',
        desc: 'Sholat Dhuha sebelum Dzuhur',
        xpReward: 40,
        target: 1,
        progress: 0,
        completed: false,
      ...
      Quest(
        id: 'quest_rawatib_two',
        desc: 'Rawatib 2x hari ini',
        xpReward: 45,
        target: 2,
        progress: 0,
        completed: false,
      ...
      Quest(
        id: 'quest_dzuhur_tepat',
        desc: 'Sholat Dzuhur tepat waktu (≤30 menit setelah adzan)',
        xpReward: 40,
        target: 1,
        progress: 0,
        completed: false,
      ...
      Quest(
        id: 'quest_maghrib_tepat',
        desc: 'Sholat Maghrib tepat waktu (≤30 menit setelah adzan)',
        xpReward: 40,
        target: 1,
        progress: 0,
        completed: false,
      ...
      Quest(
        id: 'quest_isya_hadir',
        desc: 'Jangan lewatkan sholat Isya malam ini',
        xpReward: 30,
        target: 1,
        progress: 0,
        completed: false,
      ...
      Quest(
        id: 'quest_any_three',
        desc: 'Kerjakan 3 sholat wajib hari ini (bebas yang mana)',
        xpReward: 50,
        target: 3,
        progress: 0,
        completed: false,
      ...
      Quest(
        id: 'quest_subuh_isya',
        desc: 'Kunci dua ujung hari: Subuh + Isya',
        xpReward: 60,
        target: 2,
        progress: 0,
        completed: false,
      ...
      Quest(
        id: 'quest_one_sunnah',
        desc: 'Kerjakan 1 sholat sunnah apa saja hari ini',
        xpReward: 30,
        target: 1,
        progress: 0,
        completed: false,
      ...
      Quest(
        id: 'quest_rawatib_one',
        desc: 'Rawatib 1x hari ini (qobliyah/ba\'diyah bebas)',
        xpReward: 25,
        target: 1,
        progress: 0,
        completed: false,
      ...
      Quest(
        id: 'quest_zikir_33',
        desc: 'Zikir 33x lewat tombol Daily Zikir',
        xpReward: 30,
        target: 33,
        progress: 0,
        completed: false,
    ...
    if (_cache.heroStreak.current >= 6) {
      pool.add(
        Quest(
          id: 'quest_hero_streak_7',
          desc: 'Pertahanin Hero Streak 7 hari! 🔥',
          xpReward: 200,
          target: 7,
          progress: _cache.heroStreak.current,
          completed: _cache.heroStreak.current >= 7,
          claimed: false,
        ),
      );
    }
  ...
  static Future<GameState> runDailyCheck() async {
    ...
    for (final k in tracker.keys) {
      prayerRecoveries += _countRecovery(
        state.perPrayerStreaks[k] ?? StreakState(),
        tracker[k]!,
      );
    }
  ...
  static StreakState _evalStreakMissed(
    StreakState s,
    bool logged,
    String evalDate,
  ) {
  ...
  static int _countRecovery(StreakState before, StreakState after) {
    ...
    if (after.current < before.current &&
        after.freezeAvailable == before.freezeAvailable) {
      return 1;
    }
    return 0;
  }
  ...
  static bool _allWajibLoggedForDate(GameState state, String date) {
    return wajibList.every(
      (p) => state.prayerLog.any((l) => l.date == date && l.prayer == p),
    );
  }

  static bool _prayerLoggedForDate(
    GameState state,
    String date,
    String prayer,
  ) {
  ...
  static bool get tilawahDoneToday => _cache.prayerLog.any(
    (l) => l.date == todayStr() && l.prayer == 'tilawah',
  );
  ...
  // Reward: 10% cosmetic free baru, 90% +30 XP.
  ...
  static Future<ChestRevealState?> claimDailyChest() async {
    ...
    final unownedFree = CosmeticCatalog.all
        .where(
          (c) =>
              ...
              !CosmeticCatalog.isDefault(c.id) &&
              !_cache.ownedCosmetics.contains(c.id),
        )
        .toList();
    final cosmetic = unlockCosmetic && unownedFree.isNotEmpty
        ? unownedFree[(debugChestPickIndex ??
                  _rng.nextInt(unownedFree.length)) %
              unownedFree.length]
        : null;
    final xpReward = cosmetic == null ? 30 : 0;
    final oldInfo = getLevelInfo(_cache.xp);
    ...
    await _save(
      _cache.copyWith(
        ...
        level: newInfo.level,
        ownedCosmetics: cosmetic == null
            ? _cache.ownedCosmetics
            : [..._cache.ownedCosmetics, cosmetic.id],
    ...
    return ChestRevealState(
      xpReward: xpReward,
      rewardName: cosmetic?.name ?? '30 XP',
      rewardEmoji: cosmetic?.emoji ?? '✨',
...
void main() {
  // Level curve sanity
  assert(
    GameService.xpNeededForLevel(1) == 48 ||
        GameService.xpNeededForLevel(1) == 49,
  );
